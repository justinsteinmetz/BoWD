...
const unlockedBlocks = new Set([0]);
...
function renderBlockNav() {
  ...
  blocks.forEach((block, bi) => {
    ...
    if (!unlockedBlocks.has(bi)) { btn.classList.add("locked"); btn.disabled = true; }
    btn.onclick = () => {
...
function renderQuiz(zone, sk) {
  ...
  const qs = zone.questions.map((q, qi) => {
    ...
    const feedback = answered !== undefined
      ? `<div class="feedback ${answered === q.correct ? "fb-correct" : "fb-wrong"}">${answered === q.correct ? "Correct" : "Not quite"} — ${q.explain}</div>`
  ...
  const score = allAnswered ? Object.entries(answers).filter(([qi, oi]) => oi === zone.questions[+qi].correct).length : 0;
  const scoreBadge = allAnswered ? `<div class="score-line">Score: <strong>${score} / ${zone.questions.length}</strong></div>` : "";
...
function renderCharmap(zone, sk) {
  const nodes = zone.characters.map((c, i) => `
    <div class="char-node" data-ci="${i}">
      <div class="char-icon">${(typeof CHAR_ICONS !== 'undefined' && CHAR_ICONS[c.icon]) ? CHAR_ICONS[c.icon] : c.initial || c.name[0]}</div>
...
function renderCraft(zone, sk) {
  const bk = zone.similes.map((s, si) => {
  ...
  return `<div class="craft-wrap">${bk}</div>`;
}
...
function renderCreative(zone, sk) {
  const chips = zone.chips.map(c => `<button class="prompt-chip" data-prompt="${c}">${c}</button>`).join("");
  const saved = (zoneState[sk] || {}).text || "";
  return `
...
function wireZone(section, zone, block, sk) {
    ...
    section.querySelectorAll(".opt-btn").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        if (Object.keys(answers).length === zone.questions.length) {
          const score = Object.entries(answers).filter(([i, v]) => v === zone.questions[+i].correct).length;
      ...
      node.addEventListener("click", () => {
        ...
        panel.innerHTML = `
          <div class="char-expand-inner">
            <div class="char-expand-name">${c.name} <span class="char-expand-role">${c.role}</span></div>
  ...
  if (zone.type === "craft") {
    ...
    section.querySelectorAll(".reveal-btn[data-si]").forEach(btn => {
      btn.addEventListener("click", () => {
