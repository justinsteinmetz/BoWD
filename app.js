...
function startSession(){
  ...
  setTimeout(() => {
    ...
    Object.entries(STATE.text).forEach(([group, users]) => {
      if(users[currentUser] && channel){
        channel.postMessage({
          user: currentUser,
          group,
          text: users[currentUser]
        });
      }
    });
  }, 200);
...
function renderRecall(zone){
  ...
  return `
    <div class="entry-note">
      Earlier: ${keys.map(k => `<strong id="recall-${k}">—</strong>`).join(" / ")}
    </div>
  `;
}
...
function renderAggregation(group, data){
  ...
  el.innerHTML = data.map(([k,v])=>`
    <span class="agg-item" data-val="${k}" onclick="spotlight('${group}','${k}')">
      ${k}: ${v}
    </span>
  `).join(" · ");
}
...
function showResponses(group, value){
  ...
  panel.innerHTML = shuffled.length
    ? shuffled.map(r=>`<div class="response-item">"${r}"</div>`).join("")
    : `<em style="opacity:0.5;">No responses yet</em>`;
}
